...
void ImageBMP::createBMP(unsigned int width, unsigned int height, string fileName) {
	...
	try  {
		...
		//heightInTiles = (height + tileH - 1) / tileH;

		//tiles = new int*[widthInTiles * heightInTiles];

		//for (int h = 0; h < heightInTiles; h++) {
		//	for (int w = 0; w < widthInTiles; w++) {
		//		tiles[w + widthInTiles * h] = new int[tileW * tileH];
		//	}
		//}
...
void ImageBMP::loadBMP(string inFileName) {
	...
	if (fopen_s(&file, openFile.append(".bmp").c_str(), "rb") == 0) {
		...
		//now copy over bmp pixel Color_RGB values into buffer
		this->createBMP(this->width, this->height, inFileName.append("_output")); //first allocate buffer data
		
		//read the rest of the header file until start of Color_RGB data
		for (int i = 0; i < 28; i++)
			fgetc(file);

		//calculate padding needed for image so we can extract Color_RGB properly
		...
		*/
		...
		//load texture data into tiles
		/*
		for (int atTileY = 0; atTileY < heightInTiles; atTileY++) {
			for (int atTileX = 0; atTileX < widthInTiles; atTileX++) {
				for (int y = 0; y < tileH; y++) {
					for (int x = 0; x < tileW; x++) {

						tiles[atTileX + atTileY * widthInTiles][x + y * tileW] = intbuffer[(atTileX * tileW + x) + (atTileY * tileH + y) * height];
					}
				}
			}
		}
...
void ImageBMP::convertToGrayScale() {
	for (unsigned int i = 0; i < height; i++) {
		for (unsigned int j = 0; j < width; j++) {
			Color_RGB pixel = this->getPixelColor(j, i);
...
void ImageBMP::performSobelEdgeDetection(unsigned int brightness) {
	...
	for (unsigned int y = 0; y < height; y++) {
		for (unsigned int x = 0; x < width; x++) {
			Color_RGB copy = this->getPixelColor(x, y);
